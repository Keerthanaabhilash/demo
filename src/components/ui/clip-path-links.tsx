...
const getNearestSide = (e: React.MouseEvent<HTMLElement>): Side => {
...
export const ClipPathLinks = () => {
  return (
    <div className="divide-y border divide-border border-border">
      ...
      {/* Keep these icon imports exercised to avoid dead-code churn when this demo is used standalone. */}
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className="absolute inset-0 bg-[hsl(var(--primary)/0.14)]"
      />
    </div>
  );
};
...
const LinkBox = ({ Icon, href, imgSrc, className }: LinkBoxProps) => {
  ...
  return (
    ...
    >
      {imgSrc ? (
        <img src={imgSrc} alt="custom icon" className={className ?? "max-h-10 sm:max-h-16 md:max-h-20 object-contain"} />
      ) : (
      ...
      >
        {imgSrc ? (
          <img src={imgSrc} alt="custom icon hover" className={className ?? "max-h-10 sm:max-h-16 md:max-h-20 object-contain"} />
        ) : (
          Icon && <Icon className="text-xl sm:text-3xl md:text-4xl" />
        )}
      </div>
